...
const FlowDiagram = () => {
  ...
  const stepExamples = {
    ...
    1: (
      <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 flex flex-col items-center">
        <p className="text-white/80 text-sm mb-2 text-center">&ldquo;Increase brand awareness among tech-savvy millennials&rdquo;</p>
        <div className="flex gap-2 justify-center">
          <span className="px-2 py-1 bg-white/10 text-white/80 rounded-full text-xs">Tech</span>
          <span className="px-2 py-1 bg-white/10 text-white/80 rounded-full text-xs">Lifestyle</span>
        </div>
      </div>
    ),
    2: (
          ...
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center gap-4">
                ...
                <span className="text-white/60 text-xs">{creator.followers}</span>
              </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 flex flex-col items-center">
        <div className="grid grid-cols-2 gap-3">
          <div className="text-center p-2 bg-white/5 rounded-lg">
            ...
            <div className="text-white/60 text-xs">Brand Safety</div>
          </div>
          <div className="text-center p-2 bg-white/5 rounded-lg">
          ...
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1 text-center">
              <div className="flex justify-between items-center gap-4">
                ...
                <span className="text-white/60 text-xs">{video.views}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 space-y-3 flex flex-col items-center">
        {[
          ...
          <div key={i} className="flex justify-between items-center w-full gap-4">
            ...
            <span className="px-2 py-1 bg-white/10 text-white/80 rounded-full text-xs">{idea.score}</span>
          </div>
        ))}
      </div>
    ),
        ...
        <div className="flex flex-col items-center space-y-1">
          {steps.map((step, index) => {
            ...
            return (
              <Fragment key={index}>
                <div
                  className={`transition-all duration-500 h-[120px] flex items-center justify-center w-full ${
                    isVisible
                ...
                {index < steps.length - 1 && (
                  ...
                  >
                    <div className={`h-6 border-l-2 border-dashed border-purple-400 ${
        ...
        <div className="flex flex-col items-center space-y-1">
          {steps.map((step, index) => {
            ...
            return (
              <Fragment key={index}>
                <div
                  className={`transition-all duration-500 w-full h-[120px] flex items-center justify-center ${
                    isVisible
                      ? 'opacity-100 translate-y-0'
                      : 'opacity-0 translate-y-4 pointer-events-none'
                  } ${
                ...
                >
                  <div className={`bg-white/5 backdrop-blur-sm rounded-lg p-4 w-full h-full flex items-center justify-center ${
